...
let cartItems, cartTotal, cartCount, notification;
...
localStorage.setItem("cart", JSON.stringify(cart)); // 🔥 penting
updateCart();
showNotification();
...
<textarea 
placeholder="Request your custom bouquet here 💗"
oninput="updateReq(${index}, this.value)"
>${item.req || ""}</textarea>
...
function updateReq(index,value){
    ...
    localStorage.setItem("cart", JSON.stringify(cart)); // 🔥 WAJIB
}
...
updateCart();
showNotification();
flyToCart(btn.dataset.img, popupCartBtn);
...
function goOccasion(occ){
window.location.href = "occasions.html?occasion=" + occ;
}


function loadOccasionPage(){

loadCart();
updateCart();

if(!window.location.href.includes("occasions.html")) return;

const params = new URLSearchParams(window.location.search);
const occ = params.get("occasion");

// 🔥 TARO DI SINI (SETELAH DECLARE)
if(!occ) return;

const title = document.getElementById("occasion-title");
const container = document.getElementById("occasion-container");
const search = document.getElementById("occasion-search");
const sort = document.getElementById("occasion-sort");

if(!title || !container) return;

// 🎀 Judul aesthetic
const mapTitle = {
birthday: "Perfect for Birthday 🎂",
graduation: "Perfect for Graduation 🎓",
anniversary: "Perfect for Anniversary 💖",
special: "Perfect for Special Gift 🎁",
surprise: "Perfect for Surprise 🎉"
};

title.innerText = mapTitle[occ] || "Perfect for You 💗";

fetch("data/products.json")
.then(res => res.json())
.then(data => {

let filtered = data.filter(p => p.occasion && p.occasion.includes(occ));
...
let result = data.filter(p => p.occasion && p.occasion.includes(occ));
...
document.addEventListener("DOMContentLoaded", loadOccasionPage);